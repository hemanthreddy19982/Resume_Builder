...
export async function generateDocx(resume: ResumeData): Promise<Buffer> {
  ...
  if (resume.personal.email) contactParts.push(`Email: ${resume.personal.email}`);
  if (resume.personal.phone) contactParts.push(`Phone: ${resume.personal.phone}`);
  if (resume.personal.location) contactParts.push(`Location: ${resume.personal.location}`);
  if (resume.personal.linkedin) contactParts.push(`LinkedIn: ${resume.personal.linkedin}`);
  if (resume.personal.github) contactParts.push(`GitHub: ${resume.personal.github}`);
